...
- **Vision call:**
  ...
  - **When it fails:** the daily limit or a rejected token stops the run. Any other failure makes the QC result `vision_error`: the unit becomes `needs_review` without a retry, since a new image won't fix the checker.
