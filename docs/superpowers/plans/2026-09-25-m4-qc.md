...
- **Figures expected:** ~~the sum of `figures` over the unit's characters (mascot 1, a cast entry its `figures`). They must match exactly up to 3, and within ±1 above that (§11.3).~~
  - **Changed after the live check (Task 11), at the user's decision (2026-09-26):** the expected figures are a range. Each distinct character ref counts once. The minimum is the number of distinct refs, and the maximum is the sum of their `figures`, so a group may show 1 up to its full size. A count passes when `min ≤ seen ≤ max`, with ±1 at both ends when the maximum is over 3 (§11.3 [M4]). Why: 7 of the 13 live `character_count` failures were false, from group entries used for one member and from one group listed three times (expecting 9).
- **Retry chains:**
